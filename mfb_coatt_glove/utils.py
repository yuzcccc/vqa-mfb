...
from vqa_data_layer import VQADataProvider, VQADataProviderLayer
...
def exec_validation(device_id, mode, model_name, folder, it='', visualize=False):

    caffe.set_device(device_id)
    caffe.set_mode_gpu()
    net = caffe.Net('./%s/proto_test.prototxt'%folder,model_name,caffe.TEST)

    dp = VQADataProvider(mode=mode,batchsize=config.VAL_BATCH_SIZE,folder=folder)
